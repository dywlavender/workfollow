  ...
  test('business Dart sources stay behind the design-system token boundary',
    ...
    for (final entity in root.listSync(recursive: true)) {
      ...
      for (final entry in entity.readAsStringSync().split('\n').indexed) {
        ...
        if (!tokenSource &&
            RegExp(r'\b(?:Icons|CupertinoIcons)\.').hasMatch(line)) {
          reject('use WorkFollowIcons');
        }
        if (!tokenSource && RegExp(r'Color\s*\(\s*0x').hasMatch(line)) {
          reject('use a semantic color token');
        }
        if (!tokenSource &&
            RegExp(r'\bColors\.(?:red|redAccent|green|greenAccent|grey|gray|orange|blue|blueAccent|purple|pink|indigo|teal|yellow|amber|deepOrange|lightBlue|cyan|lime|brown|blueGrey)\b')
                .hasMatch(line)) {
        ...
        if (!tokenSource &&
            RegExp(r'fontSize\s*:\s*(?:const\s+)?(?:\d+(?:\.\d+)?|\.\d+)')
                .hasMatch(line)) {
          reject('use a typography role');
        }
        if (!tokenSource &&
            RegExp(r'letterSpacing\s*:\s*-?(?:\d+(?:\.\d+)?|\.\d+)')
                .hasMatch(line)) {
        ...
        if (!tokenSource &&
            RegExp(r'BorderRadius\.(?:circular|only)\(\s*(?:const\s+)?(?:\d+(?:\.\d+)?|\.\d+)')
                .hasMatch(line)) {
          reject('use a radius role');
        }
        if (!tokenSource &&
            RegExp(r'(?:BoxShadow\(|elevation\s*:\s*(?:\d+(?:\.\d+)?|\.\d+))')
                .hasMatch(line)) {
          reject('use a surface shadow role');
        }
        if (!tokenSource &&
            RegExp(r'Duration\(\s*(?:const\s+)?milliseconds\s*:\s*\d+')
                .hasMatch(line)) {
