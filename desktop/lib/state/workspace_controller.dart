...
class WorkspaceController extends ChangeNotifier {
  ...
  bool _restoreCompletionSnapshot(
    ...
    if (spawnedId != null) {
      _tasks.removeWhere(
          (task) => task.id == spawnedId || spawnedChildIds.contains(task.id));
      unawaited(_reminders.cancel(spawnedId));
      for (final childId in spawnedChildIds) {
  ...
  UndoCommand _undoSkipOccurrence(
      TaskItem before, String spawnedId, List<String> spawnedChildIds) {
    late final UndoCommand command;
    command = UndoCommand(
      label: '撤销跳过本周期',
      execute: () {
        ...
        _tasks.removeWhere((task) =>
            task.id == spawnedId || spawnedChildIds.contains(task.id));
        unawaited(_reminders.cancel(spawnedId));
        for (final childId in spawnedChildIds) {
          unawaited(_reminders.cancel(childId));
        }
  ...
  TaskActionResult _dispatchTaskAction(String action, Object? payload) {
    switch (action) {
      ...
      case 'skipOccurrence':
        ...
        final now = DateTime.now();
        final occurrence = _spawnNextRecurrence(task, completedAt: now);
        if (occurrence == null) {
          return TaskActionResult.failure(
              'recurrence-not-supported', '无法计算下一周期');
        }
        final next = occurrence.spawn;
        ...
        _tasks = [next, ...occurrence.children, ..._tasks];
        _syncReminderFor(skipped);
        _syncReminderFor(next);
        for (final child in occurrence.children) {
          _syncReminderFor(child);
        }
        _setSelectedTaskId(next.id);
        _actionVersion += 1;
        _lastActionKind = 'skipOccurrence';
        _lastActionMessage = '已跳过本周期';
        final undo = _undoSkipOccurrence(
            task, next.id, occurrence.children.map((c) => c.id).toList());
        return _taskResult(id,
            message: _lastActionMessage,
            showFeedback: true,
            undo: undo,
  ...
  void toggleTask(String id) {
    ...
    if (completing) {
      ...
      if (occurrence != null) {
        ...
        carriedChildren = occurrence.children;
    ...
    if (spawn != null) {
      _tasks = [spawn, ...carriedChildren, ..._tasks];
      ...
      for (final child in carriedChildren) {
        _syncReminderFor(child);
      }
    }
  ...
  /// task had no date.
  ///
  /// TickTick behavior: a recurring parent carries its active children into
  /// the next occurrence, reset to incomplete — the checklist comes back
  /// instead of dying with the completed record. Child dates ride the same
  /// recurrence delta as the parent's reminder/deadline so the relative
  /// schedule survives, and carried children lose their own repeat rules so
  /// only the parent drives the chain. A recurring child spawns inside its
  /// parent instead of escaping to the root.
  ({TaskItem spawn, List<TaskItem> children})? _spawnNextRecurrence(
      TaskItem task,
    ...
    DateTime? shifted(String? raw) {
      final value = localDateTimeFromStorage(raw);
      return value == null ? null : value.add(recurrenceDelta);
    }
    ...
    final now = DateTime.now().toIso8601String();
    // The spawn consumes the current sequence number; every carried child
    // takes the following ones so ids stay collision-free in one pass.
    final spawn = TaskItem(
      id: 'task-${_taskSequence.toString().padLeft(2, '0')}',
      title: task.title,
      ...
      childOrder: task.isChildTask ? childrenOf(task.parentTaskId!).length : 0,
      createdAt: now,
      updatedAt: now,
    );
    _taskSequence += 1;
    ...
    for (final child in childrenOf(task.id)) {
      final childDue = shifted(child.dueAt);
      children.add(TaskItem(
        id: 'task-${_taskSequence.toString().padLeft(2, '0')}',
        title: child.title,
        listName: child.listName,
        bucket: taskBucketForDate(childDue),
        timeLabel:
            taskTimeLabelFor(childDue, hasTime: child.scheduledWithTime) ??
                '未安排',
        note: child.note,
        description: child.description,
        contentJson: child.contentJson,
        dueAt: childDue?.toIso8601String(),
        dueEndAt: shifted(child.dueEndAt)?.toIso8601String(),
        hasDueTime: child.hasDueTime,
        deadlineAt: shifted(child.deadlineAt)?.toIso8601String(),
        reminderAt: shifted(child.reminderAt)?.toIso8601String(),
        reminderOffsets: child.reminderOffsets,
        recurrenceType: 'NONE',
        tags: child.tags,
        sourceNoteId: child.sourceNoteId,
        attachments: child.attachments,
        convertedNoteId: child.convertedNoteId,
        priority: child.priority,
        parentTaskId: spawn.id,
        childOrder: children.length,
        createdAt: now,
        updatedAt: now,
      ));
      _taskSequence += 1;
    }
    return (spawn: spawn, children: children);
  }
  ...
  bool undoLastCompletion() {
    ...
    // removed together with the children it carried over, and the completed
    // record carries its rule again.
    ...
    if (spawnId != null && _lastCompletedRecurrenceType != null) {
      ...
      final spawnChildIds = _lastRecurrenceSpawnChildIds;
      _tasks.removeWhere(
          (item) => item.id == spawnId || spawnChildIds.contains(item.id));
      unawaited(_reminders.cancel(spawnId));
      for (final childId in spawnChildIds) {
        unawaited(_reminders.cancel(childId));
      }
    }
  ...
  void bulkCompleteSelected() {
    ...
    final spawnedChildrenBySpawn = <String, List<TaskItem>>{};
    ...
    for (final id in _multiSelectedTaskIds) {
      ...
      if (occurrence != null) {
        spawned.add(occurrence.spawn);
        spawnedChildrenBySpawn[occurrence.spawn.id] = occurrence.children;
        spawnedIds.add(occurrence.spawn.id);
    ...
    if (spawned.isNotEmpty) {
      ...
      for (final spawn in spawned) {
        prepended
          ..add(spawn)
          ..addAll(spawnedChildrenBySpawn[spawn.id] ?? const <TaskItem>[]);
      }
      _tasks = [...prepended, ..._tasks];
      for (final children in spawnedChildrenBySpawn.values) {
...
class _BulkTaskUndo {
  ...
  final List<String> spawnedChildIds;
