  ...
  static Future<String?> show(
    ...
    return showDesktopMenu<String>(anchor,
        ...
        entries: [
          ...
          const DesktopMenuEntry('today', '安排到今天', icon: WorkFollowIcons.today),
          const DesktopMenuEntry('tomorrow', '安排到明天',
              icon: WorkFollowIcons.tomorrow),
          ...
          if (task.dueAt != null || task.hasDueTime == true)
            const DesktopMenuEntry('clear-date', '清除日期',
                icon: WorkFollowIcons.clearDate),
          ...
          const DesktopMenuEntry('priority-none', '取消优先级',
              icon: WorkFollowIcons.flag),
          const DesktopMenuEntry('list', '移动到清单…', icon: WorkFollowIcons.move),
          ...
          const DesktopMenuEntry('repeat', '设置重复…',
