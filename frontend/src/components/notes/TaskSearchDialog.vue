  ...
  <Teleport to="body">
    <div v-if="open" class="dialog-backdrop task-search-backdrop" @mousedown.self="emit('close')">
      <section class="task-search-dialog" role="dialog" aria-modal="true" aria-labelledby="task-search-title">
        <header>
          <div><span class="eyebrow">关联与行动</span><h2 id="task-search-title">关联已有代办</h2></div>
          <button type="button" aria-label="关闭" @click="emit('close')"><IconX :size="18" /></button>
        </header>
        <label class="task-search-input"><IconSearch :size="17" /><input v-model="query" autofocus placeholder="搜索你有权访问的代办" /></label>
        <div class="task-search-results" role="listbox" aria-label="代办搜索结果">
          ...
          <p v-if="loading">正在读取代办…</p>
          <p v-else-if="!tasks.length">没有可关联的代办</p>
        </div>
      </section>
    </div>
  </Teleport>
