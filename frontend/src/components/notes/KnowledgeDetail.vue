...
<template>
  <section class="notes-column editor-column collaboration-reader knowledge-detail" @click="closeSettings">
    ...
    <template v-else>
      ...
      <RichTextDocument v-if="note.permissions.canEdit && collaborationSession && collaborationContentReady" ref="documentRef" v-model="contentJson" :document="collaborationSession.document" :initial-content="note.contentJson" :collaboration-document-name="`knowledge-draft:${note.id}`" class="collaboration-document" :editable="!staleDraftVersion" :seed-ready="collaborationContentReady" @ready="collaborationBodyReady = true" @initialization-error="collaborationBodyReady = false; collaborationStatus = 'error'; collaborationError = $event" @update:plain-text="plainText = $event" />
