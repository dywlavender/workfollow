...
* 点击一级入口后，第二栏由 `ContextNavigation` 按模块切换；任务栏只展示任务视图、清单、标签，笔记栏只展示笔记视图/文件夹；首页与日历/四象限/看板/习惯/统计不渲染第二栏，只保留图标 rail 作为唯一导航面（2026-09-16 收敛，见 `WorkspaceController.isSelfContainedView`）。
