...
| --- | --- | --- | --- | --- | --- | --- |
| TYPO-001 | Typography | `widgets/matrix/matrix_add_surface.dart:267` | `fontSize: 18` | `control` 或 Matrix Control Role | migrated | D1 |
| TYPO-002 | Typography | `widgets/matrix/matrix_quadrant.dart:132` | `fontSize: 11` | `caption` 或 Matrix Quadrant Label Role | migrated | D1 |
| TYPO-003 | Typography | `widgets/sidebar.dart:542` | 隐藏输入 `fontSize: 1` | 技术例外 | accepted-exception | D1 |
| TYPO-004 | Typography | 8 处 `Theme.of(...).textTheme` | Material text theme | 明确的 macOS 语义角色 | migrated | D1 |
| TYPO-005 | Typography | `screens/trash_screen.dart:118` | `letterSpacing: .45` | `none` 或有记录的 Caption tracking | migrated | D1 |
...
| ICON-001 | Icons | `widgets/task_editor_toolbar.dart:197` | `Icons.history` | `WorkFollowIcons.history` / `historyToggle` | migrated | D2 |
| ICON-002 | Icons | `widgets/task_editor_toolbar.dart:318` | `Icons.check` | `WorkFollowIcons.check` | migrated | D2 |
| ICON-003 | Icons | `widgets/task_context_menu_panel.dart:170` | `Icons.wb_twilight_outlined` | `tomorrow` 或独立语义映射 | migrated | D2 |
| ICON-004 | Icons | `widgets/task_schedule_panel.dart` | 4 个直接 chevron/circle glyph | `WorkFollowIcons` 映射 | migrated | D2 |
| ICON-005 | Icons | `widgets/task_schedule_options.dart:170` | 直接 down/close glyph | `expandMore` / `close` | migrated | D2 |
...
| GEOM-004 | Geometry | `today_screen.dart`, Matrix/Feedback 组件 | 动态约束或模块固定尺寸 | Layout contract / 模块 Metrics | migrated | D11.1 |
...
| SURF-003 | Surfaces | Quick Add、Today、Feedback、Matrix | 局部阴影或圆角组合 | `WorkFollowSurfaceTokens` / `WorkFollowShadows` | migrated | D11.1 |
...
| STATE-005 | States | `task_row.dart`、`task_list/*`、`task_document_styles.dart`、Matrix/Feedback/QuickAdd | 任务/文档/矩阵状态分支 | 共享状态 Token 与业务语义 | migrated | D11.1 |
...
| OVERLAY-005 | Overlay | `settings_panel.dart` 内部确认流程 | 真正的二级 Dialog 使用 `showDialog` | Dialog 白名单 | accepted-exception | D8 |
...
| MOTION-005 | Motion | `screens/today_screen.dart`、`widgets/task_list/*`、`widgets/matrix/*` | 状态/任务行动画 | 对应 task/list/collapse roles | migrated | D11.1 |
...
| THEME-006 | Light-Dark | `widgets/task_document_styles.dart`, `widgets/task_schedule_options.dart`, `widgets/task_schedule_panel.dart`, `widgets/task_row.dart`, `widgets/task_list/*`, `widgets/matrix/*`, `features/feedback/*` | Checklist/Schedule/Row/Matrix/Feedback 明暗分支 | 对应语义 Token 与 `WorkFollowThemeContrast` | migrated | D11.1 |
