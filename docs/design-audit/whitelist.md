...
| Feedback timing token | `WorkFollowFeedbackTiming` | 停留值已命名并集中管理，业务事件只读取语义角色 | D11.1 |
...
| 模块几何 Token | `TaskListMetrics`、`TaskScheduleMetrics`、`MatrixMetrics`、`TaskDocumentMetrics` | 并行组件的固定几何已迁移到模块事实源，业务 Widget 不保留裸数字 | D11.1 |
...
| 集成复核 | Feedback、Matrix、Task List、Schedule、Today 和 Quick Add | 并行源码已纳入分支；全部业务文件接受同一份锁定测试，不再有路径级豁免 | D11.1 |
