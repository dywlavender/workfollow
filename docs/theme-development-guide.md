...
3. **氛围背景（atmosphere）**：季节主题使用渐变、光晕和场景背景；经典主题保持普通工作台，不强制加入装饰。
...
| 季节场景 DOM、粒子和动效 | `frontend/src/components/SeasonalAtmosphere.vue` |
...
| rail | 实体 | 86% | 最左侧主导航 |
| navigation | 实体 | 82% | 文件夹、任务导航等 |
| panel | 实体 | 78% | 列表、索引面板 |
| detail | 实体 | 84% | 阅读详情、普通详情面板 |
| editor | 实体 | 64% | 笔记和任务正文编辑区 |
| input | 实体 | 86% | 输入框、选择框 |
| dialog | 实体 | 90% | 弹窗 |
| menu | 实体 | 84% | 下拉菜单、浮层 |
...
经典主题必须保持原有稳定表面；只在 `html[data-atmosphere="on"]` 下启用透明度和模糊规则。这样新增季节主题不会误伤默认、经典或纯色背景。
